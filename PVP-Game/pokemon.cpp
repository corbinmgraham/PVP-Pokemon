...
{
  effective_stat[stat_hp] = effective_stat[stat_hp] - dmg;
...
const move_db Pokemon::get_move(int i) const
{
  return moves[move_index[i]];
}

const char *Pokemon::get_move_str(int i) const
{
  if (i < 4 && move_index[i]) {
...
{
  return get_species() == p.get_species();
}
