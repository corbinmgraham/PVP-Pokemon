...
void new_hiker()
{
  ...
  for(int i = 0; i < 6; i++) {
    if (i == 0) c->captured[0] = new Pokemon(1);
    else c->captured[i] = NULL;
  }
...
void game_loop()
{
  ...
  while (!world.quit) {
    ...
    if(n) {
      if(n->defeated == 0) heap_insert(&world.cur_map->turn, c);
      else {
        world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
        world.cur_map->num_trainers--;
      }
    }
    else heap_insert(&world.cur_map->turn, c);
  }
}
...
int main(int argc, char *argv[])
{
  ...
  Item p1;
  p1.itype = pokeball;
  world.pc.Bag.push_back(&p1);

  Item p2;
  p2.ability = 10;
  p2.itype = potion;
  for(int k = 0; k < 10; k++) { world.pc.Bag.push_back(&p2); }

  Item p3;
  p3.itype = revive;
  world.pc.Bag.push_back(&p3);

  for(int i = 0; i < 6; i++) {
    world.pc.captured[i] = NULL;
  }

  world.pc.curr_pokemon = 0;
  world.pc.captured[world.pc.curr_pokemon] = new Pokemon(10);
  world.pc.captured[world.pc.curr_pokemon+1] = new Pokemon(1);
  world.pc.captured[world.pc.curr_pokemon+2] = new Pokemon(10);
  world.pc.captured[world.pc.curr_pokemon]->damage(world.pc.captured[world.pc.curr_pokemon]->get_max_hp());
